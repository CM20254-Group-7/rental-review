...
export const claimProperty = async (
  ...
  const openClaimToClose: false | { started_at: Date } = false;
...
const setPropertyOwnershipWithClose = async (
  ...
  if (!openClaimToClose) return setPropertyOwnership(propertyId, landlordId, startedAt, endedAt);
  ...
  return setPropertyOwnership(propertyId, landlordId, startedAt, endedAt);
};
