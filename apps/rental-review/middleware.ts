...
import { createMiddlewareSupabaseClient } from '@repo/supabase-client-helpers';
...
export async function middleware(request: NextRequest) {
  ...
  if (
    ...
    if (!user) {
      const redirectTo = new URL(
        `/login?redirect=${request.nextUrl.pathname}&message=${'You must be logged in to access this page.'}`,
        request.url,
      );
