...
test.describe('User 1 tests', () => {
  ...
  test('reviewing new property', async ({ page }, testInfo) => {
    ...
    // fills in date
    await page.evaluate(
      `document.getElementById("review_date").setAttribute("value", ${formattedDate})`,
    );
  ...
  test('reviewing an existing property <Existing Property> <Existing Review>', async ({
    page,
  }, testInfo) => {
    const property = properties[testInfo.workerIndex]!;
  ...
  test('reviewing prevent creating a new property for existing property <Existing Property>', async ({
    page,
  }) => {
...
test.describe('User 4 tests', () => {
  ...
  test('reviewing a property already reviewed by user <Existing Property>', async ({
    page,
  }) => {
